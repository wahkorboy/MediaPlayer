...
class SleepTimeActivity : AppCompatActivity() {
    ...
    private fun setAlarm(timeInMills: Long) {
        val launchTime: Long = when (sleep.SleepMode) {
            repeatMode[1] -> sleep.TimeInDay    // calculate time needed!!!
            repeatMode[2] -> sleep.TimeDelay     // actual value
            repeatMode[3] -> sleep.TimeAfter       // calculate time needed!!!
            else -> {
                if (sleep.TimeDelay > 0) sleep.TimeDelay else if (sleep.TimeInDay > 0) sleep.TimeInDay else 0
            }
        }
        if (launchTime>0){
